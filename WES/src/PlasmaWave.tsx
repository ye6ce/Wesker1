...
void mainImage(out vec4 C, in vec2 U) {
  ...
  // Since colors are black, we just return black with the threshold as alpha
  C = vec4(0.0, 0.0, 0.0, lum);
}
...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      program.uniforms.iTime.value = 45.0; // Fixed value for a consistent static look
      ...
      // Stop loop since it is now static
    ...
    function resize() {
      ...
      // Trigger a render frame on resize
