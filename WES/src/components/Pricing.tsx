...
const Pricing = memo(function Pricing({ onSelectService }: PricingProps) {
  return (
    <section className="relative w-full bg-brand-green py-20 md:py-32 px-6 md:px-12 lg:px-24 text-black z-10 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        >
          <h2 className="script-text text-4xl md:text-5xl mb-4 text-black">
            Our Pricing
          </h2>
          <p className="text-black font-poppins max-w-lg mx-auto opacity-70">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
          {tiers.map((tier, index) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className={`relative flex flex-col p-8 rounded-2xl border-2 shadow-[0_15px_30px_-10px_rgba(0,0,0,0.15)] ${
                tier.isPopular ? 'border-black bg-black/5' : 'border-black/10 bg-black/5'
            ...
            >
              ...
              <div className="mb-8">
                <h3 className="script-text text-2xl mb-2 text-black">{tier.name}</h3>
                <div className="flex items-baseline gap-2 mb-4 h-12">
                  <span className="font-poppins text-4xl font-bold tracking-tight text-black">
                    <AnimatedPrice price={tier.price} name={tier.name} />
                    {tier.suffix}
                  </span>
                </div>
                <p className="text-black text-sm leading-relaxed opacity-70">{tier.description}</p>
              </div>
                ...
                <ul className="flex flex-col gap-4">
                  {tier.features.map((feature) => (
                    <li key={feature} className="flex items-center gap-3">
                      <div className={`flex items-center justify-center w-5 h-5 rounded-full border border-black ${
                        tier.isPopular ? 'bg-black text-brand-green' : 'bg-transparent text-black'
                      }`}>
              ...
              <div className="mt-10">
                <button 
                  ...
                  className={`w-full py-4 rounded-full font-poppins text-sm font-bold tracking-wide transition-all border-2 border-black ${
                  tier.isPopular 
                    ? 'bg-black text-brand-green hover:bg-transparent hover:text-black' 
                    : 'bg-transparent text-black hover:bg-black hover:text-brand-green'
                }`}>
                  {tier.buttonText}
                </button>
              </div>
