...
type GlowColor = 'green' | 'darkGreen';
...
const GlowingOrbitPath = memo(({ radius, glowColor = 'green', animationDelay = 0 }: GlowingOrbitPathProps) => {
  const glowColors = {
    green: {
      primary: 'rgba(210, 255, 0, 0.4)',
      secondary: 'rgba(210, 255, 0, 0.1)',
      border: 'rgba(210, 255, 0, 0.2)'
    },
    darkGreen: {
      primary: 'rgba(163, 230, 53, 0.3)',
      secondary: 'rgba(163, 230, 53, 0.05)',
      border: 'rgba(163, 230, 53, 0.15)'
    }
  };

  const colors = glowColors[glowColor] || glowColors.green;
...
export default function OrbitingSkills() {
  ...
  const orbitConfigs: Array<{ radius: number; glowColor: GlowColor; delay: number }> = [
    { radius: isMobile ? 80 : 100, glowColor: 'green', delay: 0 },
    { radius: isMobile ? 140 : 180, glowColor: 'darkGreen', delay: 1.5 }
  ];

  // Helper function to get radius based on screen size
  const getRadius = (baseRadius: number) => {
    return isMobile ? baseRadius * 0.75 : baseRadius;
  };

  return (
    <main className="w-full flex items-center justify-center overflow-hidden py-10 md:py-20">
      {/* Background pattern */}
      <div className="absolute inset-0 opacity-10">
        <div 
          className="w-full h-full" 
          style={{ 
            backgroundImage: `radial-gradient(circle, rgba(210,255,0,0.1) 1px, transparent 1px)`, 
      ...
      >
        ...
        <div className="w-16 h-16 md:w-24 md:h-24 bg-gradient-to-br from-gray-700 to-gray-900 rounded-2xl flex items-center justify-center z-10 relative shadow-2xl border border-white/5">
          <div className="absolute inset-0 rounded-2xl bg-brand-green/20 blur-xl animate-pulse"></div>
          <div className="absolute inset-0 rounded-2xl bg-brand-green/5 blur-2xl animate-pulse" style={{ animationDelay: '1s' }}></div>
          <div className="relative z-10">
            <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "32" : "48"} height={isMobile ? "32" : "48"} viewBox="0 0 24 24" fill="none" stroke="url(#gradient-central)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <defs>
        ...
        {skillsConfig.map((config) => {
          const radius = getRadius(config.orbitRadius);
          const angle = time * config.speed + (config.phaseShift || 0);
          const x = Math.cos(angle) * radius;
          const y = Math.sin(angle) * radius;
          const size = isMobile ? config.size * 0.9 : config.size;
          ...
          return (
            ...
            >
               <div className="relative w-full h-full p-2 bg-gray-800/90 backdrop-blur-md rounded-xl flex items-center justify-center shadow-lg border border-white/5">
